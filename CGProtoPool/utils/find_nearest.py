...
from CGProtoPool.utils.utils import compute_rf_prototype, compute_proto_layer_rf_info_v2, makedir, find_high_activation_crop
...
class ImagePatch:
    ...
    def __init__(self, patch, label, distance,
                 original_img=None, act_pattern=None, patch_indices=None, protoL=None):
        self.patch = patch
        self.label = label
...
def find_k_nearest_patches_to_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    layer_filter_sizes, layer_strides, layer_paddings = prototype_network.features.conv_info()
    protoL_rf_info = compute_proto_layer_rf_info_v2(224, layer_filter_sizes, layer_strides, layer_paddings, prototype_kernel_size=1)
    ...
    for idx, (search_batch_input, search_y) in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            proto_dist_torch = prototype_network.prototype_distances(search_batch)
            protoL_input_torch = prototype_network.conv_features(search_batch)

        protoL_input_ = np.copy(protoL_input_torch.detach().cpu().numpy())
        proto_dist_ = np.copy(proto_dist_torch.detach().cpu().numpy())

        for img_idx, distance_map in enumerate(proto_dist_):
            for j in assigned_prototypes:
                # find the closest patches in this batch to prototype j
                closest_patch_distance_to_prototype_j = np.amin(distance_map[j])
                closest_idx_i, closest_idx_j = np.unravel_index(np.argmin(distance_map[j], axis=None), distance_map[j].shape)
                closest_protoL = protoL_input_[0][:, closest_idx_i, closest_idx_j]
    ...
    for j in assigned_prototypes:
        ...
        if full_save:
            ...
            for i, patch in enumerate(heaps[j]):
                ...
                # save the protoL vector
                np.save(os.path.join(dir_for_saving_images,
                                     'nearest-' + str(i+1) + '_protoL.npy'),
                        patch.protoL)
    ...
    labels_all_prototype = np.array([[patch.label for patch in heaps[j]] for j in assigned_prototypes])
