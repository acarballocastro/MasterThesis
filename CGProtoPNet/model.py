...
from CGProtoPNet.utils.resnet_features import resnet18_features, resnet34_features, resnet50_features, resnet101_features, resnet152_features
from CGProtoPNet.utils.densenet_features import densenet121_features, densenet161_features, densenet169_features, densenet201_features
from CGProtoPNet.utils.vgg_features import vgg11_features, vgg11_bn_features, vgg13_features, vgg13_bn_features, vgg16_features, vgg16_bn_features,\
                         vgg19_features, vgg19_bn_features

from CGProtoPNet.utils.receptive_field import compute_proto_layer_rf_info_v2
...
class PPNet(nn.Module):
    ...
    def _l2_convolution(self, x):
        ...
        '''
        x2 = x ** 2
        x2_patch_sum = F.conv2d(input=x2, weight=self.ones) # sun of the squares of the channels per pixel 
        # shape (batch, num_prototypes, H, D) with dim 1 repeated 

        p2 = self.prototype_vectors ** 2
        p2 = torch.sum(p2, dim=(1, 2, 3)) # sum of the squares of the prototypes
        # p2 is a vector of shape (num_prototypes,)
        # then we reshape it to (num_prototypes, 1, 1)
        p2_reshape = p2.view(-1, 1, 1)

        xp = F.conv2d(input=x, weight=self.prototype_vectors) # shape (batch, num_prototypes, H, D)
    ...
    def set_last_layer_incorrect_connection(self):
        ...
        self.l1_mask = 1 - torch.abs(weights).cuda()
