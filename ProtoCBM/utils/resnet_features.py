...
# Paths to download the pretrained models 
model_urls = {
...
# Local paths to the pretrained models if they are stored
model_local_paths = {
...
def resnet34_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        my_dict = torch.load(model_local_paths['resnet34'])
        my_dict.pop('fc.weight')
        my_dict.pop('fc.bias')
        model.load_state_dict(my_dict, strict=False)
    return model
...
def resnet50_features(pretrained=False, inat=False, **kwargs):
    ...
    if pretrained:
        if inat:
            ...
            model_dict = torch.load(model_local_paths['resnet50Nat'])
            ...
            my_dict = torch.load(model_local_paths['resnet50'])
            my_dict.pop('fc.weight')
            my_dict.pop('fc.bias')
            model.load_state_dict(my_dict, strict=False)
    return model
...
def resnet152_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        my_dict = torch.load(model_local_paths['resnet152'])
        my_dict.pop('fc.weight')
        my_dict.pop('fc.bias')
        model.load_state_dict(my_dict, strict=False)
    return model
